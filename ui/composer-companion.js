...
const ComposerCompanion = (function () {
  ...
  function _renderBanner(anchor, risk) {
    ...
    // Reason-based, plain-language explanation (per product direction): tell the
    // user WHY this might get cut off, not just that it might.
    let body;
    if (risk.reason === 'long-chat-and-big-ask') body = I18N.t('inpage.truncate.reasonLongChatBig');
    else if (risk.reason === 'big-ask' && risk.multiPart) body = I18N.t('inpage.truncate.body', { n });
    else if (risk.reason === 'big-ask') body = I18N.t('inpage.truncate.reasonBigAsk');
    else if (risk.longChat) body = I18N.t('inpage.truncate.reasonLongChat');
    else body = I18N.t('inpage.truncate.bodyGeneric');
