...
const Dispatcher = (function () {
  ...
  async function init(registry) {
    ...
    console.log('[tm] Token Monitor initializing on', host);
    ...
    try {
      ...
      if (tokenMonitorSettings) state.settings = tokenMonitorSettings;
    } catch (_) {}
    // Keep settings fresh if popup changes them
    try {
      chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'sync' && changes.tokenMonitorSettings) {
          state.settings = changes.tokenMonitorSettings.newValue || {};
          _emit();
        }
      });
    } catch (_) {}
    ...
    try { state.model = state.platform.getActiveModel(); }
      ...
      state.model = { id: 'unknown', displayName: 'Unknown', contextWindow: 200000, maxOutputTokens: 8192, burnRateMultiplier: 1, extendedThinking: false };
    }
  ...
  function assessTruncationRisk(draft) {
    const model = state.model || { contextWindow: 200000, maxOutputTokens: 8192 };
    ...
    const outputEst = Math.min(outputPred.predictedTokens, model.maxOutputTokens);

    const remaining = model.contextWindow - ctxTokens;
    const needed = inputEst + outputEst;
    const willTruncate = needed > remaining * 0.85;

    let suggestion = null;
    let questionsExtracted = null;
    if (willTruncate) {
      questionsExtracted = extractQuestions(draft);
      if (questionsExtracted && questionsExtracted.length > 1) {
        suggestion = 'Try asking just the first question — you\'ll get a fuller answer.';
      } else {
        suggestion = 'Consider starting a new chat for this question.';
      }
    }
